...
def visualize_tuned_result(tuned_result):
    ...
    for result in tuned_result.run_results:
        row = {key: result.params.get(key) for key in result.params if key in ('score', 'chunk_size', 'top_k')}
    ...
    if not df_results.empty:
        ...
        if not filtered_data.empty:
            ...
            chart = alt.Chart(filtered_data).mark_line(point=True).encode(
                ...
                color=alt.Color(f'{first_param}:N', legend=alt.Legend(title=f"{first_param.capitalize()}"))
            ).properties(
