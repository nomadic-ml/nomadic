...
class Experiment(BaseModel):
        ...
        for i, prompt_variant in enumerate(prompt_variants):
            ...
            if not self.evaluation_dataset:
                ...
                for j, example in enumerate(self.evaluation_dataset):
                    ...
                    # Update prompt to include sentence-by-sentence selfcheck scores, if selfcheck-prompt is the prompt approach
                    if "selfcheck-prompt" in prompt_tuner.prompt_approaches:
                        selfcheck_prompt = self._construct_selfcheck_prompt(example)
                        answer = example.get("response", None)
                        openai_api_key = (
                            self.model.api_keys.get("OPENAI_API_KEY")
                            if hasattr(self.model, "api_keys")
                            else None
                        )
                        full_prompt = prompt_tuner.update_prompt_with_selfcheck_prompt(selfcheck_prompt, prompt_variant, answer, openai_api_key)
                    full_prompt = self._construct_prompt(full_prompt, example)
                    completion_response: CompletionResponse = self.model.run(
                        prompt=full_prompt,
                        parameters=openai_params,
                    )
    ...
    def _construct_selfcheck_prompt(
        self,
        example: Dict[str, str] = None,
        context: str = "",
        instruction: str = "",
        ...
        # Determine values for query/question (interchangeable)
        query_value = example.get("query", query) or query or question

        # Return prompt for getting self-check-GPT samples
        return ("Context: " + (example.get("context", context) or context) + "\nQuery: " + query_value)
