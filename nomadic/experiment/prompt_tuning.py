...
class PromptTuner(BaseModel):
    prompt_approaches: List[str] = Field(
        default=["None"],
    ...
    def generate_prompt_variants(self, client, user_prompt_request, max_retries: int = 3, retry_delay: int = 5):
        ...
        combinations = product(
            self.prompt_approaches or [None],
    ...
    def update_prompt_with_selfcheck_prompt(
        self, prompt:str, prompt_variant: str, answer: str, openai_api_key: str,
    ) -> str:
            ...
            print("ANSWER SENTENCES: ", sentences)
            if "selfcheck-prompt" in self.prompt_approaches:
                    samples = []
                    for i in range(3):
                        response = client.chat.completions.create(
                        model="gpt-4o",
                    ...
                    generated_sample = response.choices[0].message.content.strip()
                    samples.append(generated_sample)
                    print("PROMPT FOR GENERATING SELFCHECK SAMPLES: ", prompt)
                    print("GENERATED SAMPLE: ", generated_sample)
                    selfcheck_prompt = SelfCheckAPIPrompt(client_type="openai", model=DEFAULT_OPENAI_MODEL)
                    sent_scores_prompt = selfcheck_prompt.predict(
                        sentences = sentences,                          # list of sentences
                        sampled_passages = samples, # list of sampled passages
                    )
                    prompt_variant += ("""
                        Here is the list of scores where each item is a sentence-by-sentence score of the answer from the SelfCheckGPT approach.
                        The scores are produced as following:
                        Scores are a sentence-by-sentence assessment on a scale of 0 to 1 of information consistency.
                        The judge outputs either 'Yes' or 'No' >95 percent of the time, while any remaining outputs can be set to N/A.
                        The output is converted to score: Yes -> 0.0, No -> 1.0, N/A -> 0.5."""
                         +
                        "List of scores: " + str(sent_scores_prompt) +
                        "Incorporate these scores into your judgement of hallucination on the overall answer."
                    )
    ...
    def _create_system_message(
        ...
        approach_instructions = {
            ...
            "selfcheck-prompt": "Also include a list of scores where each item is a sentence-by-sentence score from SelfCheckGPT, a simple sampling-based approach that can be used to fact-check the responses of black-box models in a zero-resource fashion. SelfCheckGPT leverages the simple idea that if an LLM has knowledge of a given concept, sampled responses are likely to be similar and contain consistent facts. However, for hallucinated facts, stochastically sampled responses are likely to diverge and contradict one another."
        }
    ...
    def update_params(self, params: Dict[str, Any]):
        if "prompt_tuning_approach" in params:
            self.prompt_approaches = [params["prompt_tuning_approach"]]
    ...
    def __str__(self):
        return f"PromptTuner(approaches={self.prompt_approaches}, complexities={self.prompt_complexities}, focuses={self.prompt_focuses})"
